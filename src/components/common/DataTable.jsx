export default function DataTable({ columns, rows, keyField = 'id', emptyLabel = 'No records to show' }) {
  ...
  return (
    <div className="scroll-thin overflow-x-auto rounded-md border border-border dark:border-night-border">
      <table className="w-full min-w-[640px] border-collapse text-left text-sm">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row[keyField]} className="border-b border-border last:border-0 hover:bg-surface-muted/60 dark:border-night-border dark:hover:bg-night-muted/60">
              {columns.map((col) => (
